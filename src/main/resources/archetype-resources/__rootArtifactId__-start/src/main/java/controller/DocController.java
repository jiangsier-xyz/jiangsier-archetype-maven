...
public class DocController {
    ...
    private static final Pattern JSON_INDEX_PATTERN = Pattern.compile("${symbol_escape}${symbol_escape}[(${symbol_escape}${symbol_escape}d+)${symbol_escape}${symbol_escape}]");
    ...
    public String switchRenderer(HttpServletRequest request) {
        ...
        if (StringUtils.isNotBlank(ref)) {
            String[] urlComponents = ref.split("[?${symbol_pound}]")[0].split("/");
    ...
    @ResponseBody
    @LongPeriodCache(keyBy = "'DocController::localizedApiDocs_' + ${symbol_pound}p1 + '_' + ${symbol_pound}p2")
    ...
    private JsonNode replaceValue(JsonNode value, Map.Entry<Object, Object> property) {
        ...
        return switch(value.getNodeType()) {
            ...
            case NUMBER -> switch (propertyValue) {
                case Integer intValue -> IntNode.valueOf(intValue);
                case Float floatValue -> FloatNode.valueOf(floatValue);
                case Double doubleValue -> DoubleNode.valueOf(doubleValue);
                case Long longValue -> LongNode.valueOf(longValue);
                case Short shortValue -> ShortNode.valueOf(shortValue);
                case BigInteger bigIntegerValue -> BigIntegerNode.valueOf(bigIntegerValue);
                case BigDecimal bigDecimalValue -> DecimalNode.valueOf(bigDecimalValue);
                default -> null;
            };
            default -> null;
        };
    ...
    private Triple<JsonNode, Object, JsonNode> getNode(JsonNode node, String path) {
        ...
        String[] parts = path.split("${symbol_escape}${symbol_escape}.", 2);
        String key = parts[0];
