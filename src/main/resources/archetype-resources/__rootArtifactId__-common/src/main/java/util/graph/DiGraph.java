...
public class DiGraph<T> implements Graph<T> {
        ...
        public String toDot(Function<T, String> converter) {
            if (CollectionUtils.isEmpty(successors)) {
                if (CollectionUtils.isEmpty(predecessors)) {
                    return converter.apply(o) + ";${symbol_escape}n";
                } else {
                    return "";
                }
            } else {
                return successors.stream()
                        .map(s -> converter.apply(o) + " -> " + converter.apply(s.getData()) + ";${symbol_escape}n")
    ...
    @Override
    public String toDot(Function<T, String> converter) {
        StringBuilder builder = new StringBuilder("digraph{${symbol_escape}n");
