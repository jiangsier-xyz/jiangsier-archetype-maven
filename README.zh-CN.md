...
> [mysql-local.sh](https://github.com/jiangsier-xyz/jiangsier-archetype-maven/blob/main/src/main/resources/archetype-resources/bin/mysql-local.sh) 和 [redis-local.sh](https://github.com/jiangsier-xyz/jiangsier-archetype-maven/blob/main/src/main/resources/archetype-resources/bin/redis-local.sh) 可以帮助你运行/停止一个本地 MySQL 和 Redis，希望能有助于你的调试。
...
缓存配置在 [cache-config.yml](https://github.com/jiangsier-xyz/jiangsier-archetype-maven/blob/main/src/main/resources/archetype-resources/__rootArtifactId__-start/src/main/resources/cache-config.yml)，这些缓存可以通过对应的 `@XxxPeriodCacheEvict` 注解进行清除，也可以使用常规的缓存注解清理（这时你可能需要用到它们的名字“shortPeriod”、“middlePeriod”、“longPeriod”）。
...
**OpenAPI 规范**<br>
awesome-app 通过 [spring-doc](https://springdoc.org/) 框架支持 [OpenAPI 规范描述](https://swagger.io/specification/)。
...
当然，从运维的角度，也许您更希望购买有 SLA(Service Level Agreement) 保障的云服务，那么只需要设置 [Helm 配置](https://github.com/jiangsier-xyz/jiangsier-archetype-maven/blob/main/src/main/resources/archetype-resources/app-meta/helm-config/values.yaml)参数为
```yaml
bitnami:
  mysql:
    enabled: false
...
awesome-app 默认部署 [bitnami/mysql](https://artifacthub.io/packages/helm/bitnami/mysql) 到同一命名空间，这时，MySQL url 默认是 `jdbc:mysql://awesome-app-mysql:3306/accounts?useUnicode=true&characterEncoding=utf-8`
